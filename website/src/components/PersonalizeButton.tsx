...
export default function PersonalizeButton(): JSX.Element | null {
  ...
  const handlePersonalize = () => {
    ...
    if (currentPath.includes('/physical-ai-humanoid-robotics-book/docs-urdu/')) {
      const pagePath = currentPath.replace('/physical-ai-humanoid-robotics-book/docs-urdu/', '');
      const newPath = `/physical-ai-humanoid-robotics-book/docs-urdu-${user.background_type}/${pagePath}`;
      history.push(newPath + location.search + location.hash);
    } else if (currentPath.includes('/physical-ai-humanoid-robotics-book/docs/')) {
      const pagePath = currentPath.replace('/physical-ai-humanoid-robotics-book/docs/', '');
      const newPath = `/physical-ai-humanoid-robotics-book/docs-${user.background_type}/${pagePath}`;
      history.push(newPath + location.search + location.hash);
